...
void processarLinha(String linha) {
    ...
    if (linha.startsWith("OTA_SIZE ")) {
        ...
        statusRecebido = false; // limpa antes de mandar, pra nao perder uma falha imediata
    ...
    if (linha == "OTA_END") {
        ...
        if (bytesRecebidos != tamanhoTotalEsperado) {
            ...
            // Nao reseta statusRecebido aqui - se uma falha ja tiver
            // chegado antes (ex.: no meio do envio dos dados), ela
            // continua valendo em vez de ser descartada.
